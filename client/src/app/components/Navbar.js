...
import { 
  ...
  FiChevronDown // added chevron down icon
} from 'react-icons/fi';
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsMobileMenuOpen(false)} className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[60]" />
            <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} className="fixed right-0 top-0 bottom-0 w-full max-w-sm bg-white z-[70] p-6 flex flex-col">
              <div className="flex justify-between items-center mb-8 pb-4 border-b border-[#D9EEF2]">
                <div>
                ...
                <button onClick={() => setIsMobileMenuOpen(false)} className="p-2 bg-[#F4FAFB] rounded-lg"><FiX size={20} /></button>
              </div>
              ...
              <div className="space-y-2 flex-grow overflow-y-auto">
                {navLinks.map((link) => (
                  <Link 
                    ...
                    className={`flex items-center gap-3 p-4 rounded-xl ${isActive(link.href) ? 'bg-[#18606D]/10 text-[#18606D]' : 'text-[#64748B]'}`}
                  >
                ...
                <button 
                  ...
                  className="w-full flex items-center gap-3 p-4 rounded-xl text-[#64748B] hover:bg-[#F4FAFB] transition"
                >
                  <FiShoppingCart /> <span className="font-medium">Cart ({cartCount})</span>
                </button>
              </div>

              <div className="space-y-3 mt-auto pt-6 border-t border-[#D9EEF2]">
                {user ? (
                  <button 
                    onClick={handleLogout}
                    className="w-full border-2 border-red-500 text-red-500 p-4 rounded-xl flex items-center justify-center gap-2 font-semibold"
                  >
                    <FiLogOut /> Logout
                  </button>
                ) : (
                  <Link href="/login" onClick={() => setIsMobileMenuOpen(false)}>
                    <button className="w-full border-2 border-[#18606D] text-[#18606D] p-4 rounded-xl flex items-center justify-center gap-2 font-semibold">
                      <FiUser /> Login
                    </button>
                  </Link>
                )}
                
                <button className="w-full bg-gradient-to-r from-[#18606D] to-[#2A7F8F] text-white p-4 rounded-xl font-semibold flex items-center justify-center gap-2">
